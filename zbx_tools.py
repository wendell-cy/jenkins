...
class zabbix_api:
    ...
    try: 
      ...
      for group in response['result']: 
        if len(hostgroupName)==0: 
          ...
          print "hostgroup: \033[31m%s\033[0m\tgroupid : %s" %(group['name'],group['groupid']) self.hostgroupID = group['groupid'] 
  ...
  def template_get(self,templateName=''): 
    ...
    try: 
      ...
      response = json.loads(result.read()) r
      esult.close() 
      #print response 
      if not len(response['result']): 
        print "\033[041m %s \033[0m is not exist" % templateName return False for template in response['result']: if len(templateName)==0: print "template : %s \t id : %s" % (template['name'], template['templateid']) 
      else: 
...
if __name__ == "__main__": 
  ...
  parser.add_argument('-T','--template',nargs='?',dest='listtemp',default='template',help='查询模板信息') parser.add_argument('-A','--add-group',nargs=1,dest='addgroup',help='添加主机组') 
